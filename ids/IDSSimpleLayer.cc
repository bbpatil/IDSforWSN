...
void IDSSimpleLayer::initialize(int stage){
	...
	if (stage==0){
		...
		enabledDropperDetection = par("enabledDropperDetection");
...
void IDSSimpleLayer::handleSelfMsg(cMessage *msg) {
    ...
    } else if (msg->getKind() == IDS_DROPPER_RESPONSES_COLLECTION_TIMER) {
        debugEV << "It is time to evaluate dropper responses about node " << msg->getName() << endl;
        evaluateDropperResponses(atoi(msg->getName()) );
        delete msg;
    } else if (msg->getKind() == IDS_DELAYER_RESPONSES_COLLECTION_TIMER) {
        debugEV << "It is time to evaluate delayer responses about node " << msg->getName() << endl;
        evaluateDelayerResponses(atoi(msg->getName()) );
        delete msg;
    } else {
        delete msg;
    }
...
void IDSSimpleLayer::responseVotingRequests() {
    ...
    while (!responseBuffer.empty()) {
        ...
        if (forwardersMap.count(responseBuffer.front().suspiciousAddr)>0 && forwardersMap[responseBuffer.front().suspiciousAddr].packetsLastWindowReceived > fwdMinPacketsReceived) {
            ...
            if (responseBuffer.front().attackKind == DROPPER) {
                idsResponsePkt->setIsAttacker(forwardersMap[responseBuffer.front().suspiciousAddr].isDropperLocal);
                idsResponsePkt->setAttackKind(DROPPER);
            } else if (responseBuffer.front().attackKind == DELAYER) {
                idsResponsePkt->setIsAttacker(forwardersMap[responseBuffer.front().suspiciousAddr].isDelayerLocal);
                idsResponsePkt->setAttackKind(DELAYER);
            }
...
void IDSSimpleLayer::askNeighborsForDropping(int nodeAddr) {
    ...
    if (forwardersMap[nodeAddr].isWaitingForDropperResponses == true) {
        debugEV << "Already waiting for dropper responses from another window" << endl;
        return;
    }

    debugEV << "Node " << nodeAddr << " is assumed to be a dropper => ask neighbors " << endl;
...
//    idsResponsesCollectionTimer = new cMessage("" + nodeAddr, IDS_RESPONSES_COLLECTION_TIMER);
    ...
    idsResponsesCollectionTimer = new cMessage(kind, IDS_DROPPER_RESPONSES_COLLECTION_TIMER);
    scheduleAt(simTime() + collectingTime, idsResponsesCollectionTimer);

    forwardersMap[nodeAddr].isWaitingForDropperResponses = true;
}
...
void IDSSimpleLayer::askNeighborsForDelay(int nodeAddr) {
    ...
    debugEV << "Node " << nodeAddr << " is assumed to be a delayer => ask neighbors " << endl;

    // Here we have to send a packet to ask neighbors for decision about the node -> this is done by IDS sender
    IDSVotingPkt *idsDelayPkt = new IDSVotingPkt();

    idsDelayPkt->setDestAddr(LAddress::L3BROADCAST);
    idsDelayPkt->setSrcAddr(net->getNetwAddr());
    idsDelayPkt->setNodeID(nodeAddr);
    idsDelayPkt->setAttackKind(DELAYER);
...
//    idsResponsesCollectionTimer = new cMessage("" + nodeAddr, IDS_RESPONSES_COLLECTION_TIMER);
    char kind[5];
    sprintf(kind, "%d", nodeAddr);
    idsDelayResponsesCollectionTimer = new cMessage(kind, IDS_DELAYER_RESPONSES_COLLECTION_TIMER);
...
void IDSSimpleLayer::votingResponse(IDSVotingPkt* idsPkt) {
    ...
    responseBuffer.push(ResponseEntry(idsPkt->getSrcAddr(), idsPkt->getNodeID(), idsPkt->getAttackKind()));
...
void IDSSimpleLayer::recordResponse(IDSVotingResponsePkt* idsResponsePkt) {
    // Dropper case
    if (idsResponsePkt->getAttackKind() == DROPPER) {
        if (idsResponsePkt->getIsAttacker()) {
            forwardersMap[idsResponsePkt->getNodeID()].positiveDropperResponses++;
        } else {
            forwardersMap[idsResponsePkt->getNodeID()].negativeDropperResponses++;
        }
        debugEV << "IDS dropper responses about node " << idsResponsePkt->getNodeID() << ": " <<
                forwardersMap[idsResponsePkt->getNodeID()].positiveDropperResponses << " positive responses and " <<
                forwardersMap[idsResponsePkt->getNodeID()].negativeDropperResponses << " negative responses." << endl;
    }
    // Delayer case
    if (idsResponsePkt->getAttackKind() == DELAYER) {
        if (idsResponsePkt->getIsAttacker()) {
            forwardersMap[idsResponsePkt->getNodeID()].positiveDelayerResponses++;
        } else {
            forwardersMap[idsResponsePkt->getNodeID()].negativeDelayerResponses++;
        }
        debugEV << "IDS delayer responses about node " << idsResponsePkt->getNodeID() << ": " <<
                forwardersMap[idsResponsePkt->getNodeID()].positiveDelayerResponses << " positive responses and " <<
                forwardersMap[idsResponsePkt->getNodeID()].negativeDelayerResponses << " negative responses." << endl;
    }
}

/**
 * This is used to evaluate responses about dropper node when the collection time is over
 */
void IDSSimpleLayer::evaluateDropperResponses(int nodeID) {
    debugEV << "We are going to evaluate dropper responses about node " << nodeID << endl;
    debugEV << "IDS dropper responses about node " << nodeID << ": " <<
                forwardersMap[nodeID].positiveDropperResponses << " positive responses and " <<
                forwardersMap[nodeID].negativeDropperResponses << " negative responses." << endl;
    debugEV << "IDS dropper responses about node " << nodeID << ": " <<
                forwardersMap[nodeID].positiveDropperResponses << " positive responses and " <<
                forwardersMap[nodeID].negativeDropperResponses << " negative responses." << endl;

    // In case of no response
    if (forwardersMap[nodeID].positiveDropperResponses == 0 && forwardersMap[nodeID].negativeDropperResponses == 0) {
        forwardersMap[nodeID].isWaitingForDropperResponses = false;
        return;
    }
    ...
    forwardersMap[nodeID].positiveDropperResponses = 0;
    forwardersMap[nodeID].negativeDropperResponses = 0;

    forwardersMap[nodeID].isWaitingForDropperResponses = false;
}

/**
 * This is used to evaluate responses about delayer node when the collection time is over
 */
void IDSSimpleLayer::evaluateDelayerResponses(int nodeID) {
    debugEV << "We are going to evaluate delayer responses about node " << nodeID << endl;
    debugEV << "IDS delayer responses about node " << nodeID << ": " <<
                forwardersMap[nodeID].positiveDelayerResponses << " positive responses and " <<
                forwardersMap[nodeID].negativeDelayerResponses << " negative responses." << endl;
    debugEV << "IDS delayer responses about node " << nodeID << ": " <<
                forwardersMap[nodeID].positiveDelayerResponses << " positive responses and " <<
                forwardersMap[nodeID].negativeDelayerResponses << " negative responses." << endl;

    // In case of no response
    if (forwardersMap[nodeID].positiveDelayerResponses == 0 && forwardersMap[nodeID].negativeDelayerResponses == 0) {
        forwardersMap[nodeID].isWaitingForDelayResponses = false;
        return;
    }
    ...
    if ( ( ( (double)forwardersMap[nodeID].positiveDelayerResponses /
        ...
        forwardersMap[nodeID].isDelayerGlobal = true;
    }

    // Reset the responses for next time.
    forwardersMap[nodeID].positiveDelayerResponses = 0;
    forwardersMap[nodeID].negativeDelayerResponses = 0;
